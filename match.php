...
<body>
    ...
    <!-- Header -->
    <header>
        <a href="adminhome.php" class="back-button">
            <i class="fas fa-arrow-left"></i> Dashboard
        </a>
        <h1>Match Management</h1>
        <p>View, search, and analyze match information</p>
    </header>

    <!-- Navbar -->
    <nav>
        <a href="input_match.html"><i class="fas fa-plus-circle"></i> Add Match</a>
        <a href="update_match.php"><i class="fas fa-edit"></i> Update Match</a>
        <a href="delete_match.php"><i class="fas fa-trash"></i> Delete Match</a>
        <a href="export_matches.php"><i class="fas fa-file-export"></i> Export Data</a>
    </nav>
                ...
                <tbody>
                    ...
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                ...
                                <td>" . $row["away_team_id"] . "</td>
                                <td>" . $row["referee"] . "</td>
                                <td>" . $row["full_time"] . "</td>
                                <td>" . $row["home_score"] . "</td>
                                <td>" . $row["away_score"] . "</td>
                                <td>" . $row["home_penalty"] . "</td>
                                <td>" . $row["away_penalty"] . "</td>
                                <td class='action-cell'>
                                    <button class='action-btn view-btn' onclick='viewMatch(" . $row["match_id"] . ")'>
                                        <i class='fas fa-eye'></i>
                                    </button>
                                    <button class='action-btn edit-btn' onclick='editMatch(" . $row["match_id"] . ")'>
                                        <i class='fas fa-edit'></i>
                                    </button>
                                    <button class='action-btn delete-btn' onclick='deleteMatch(" . $row["match_id"] . ")'>
    ...
    <footer>
        <div class="footer-links">
            ...
            <a href="update_match.php">Update Match Info</a>
            <a href="delete_match.php">Delete Match</a>
    ...
    <script>
        ...
        function viewMatch(matchId) {
            window.location.href = `view_match.php?id=${matchId}`;
        }

        // Edit match function
        function editMatch(matchId) {
            window.location.href = `update_match.php?id=${matchId}`;
        }

        // Delete match function
        function deleteMatch(matchId) {
            if (confirm(`Are you sure you want to delete match #${matchId}?`)) {
                window.location.href = `delete_match.php?id=${matchId}`;
            }
        }

        // Pagination configuration
        const rowsPerPage = 10; // Number of matches to show per page
        let currentPage = 1;
        let filteredRows = [];
