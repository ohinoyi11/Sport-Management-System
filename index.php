<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CUSTECH SPORT</title>

    <!-- font awesome integration -->
    <script src="https://kit.fontawesome.com/fd0753639b.js" crossorigin="anonymous"></script>

    <!-- stylesheet integration -->
    <link rel="stylesheet" href="style.css">
</head>
<body>
    <section class="navbar">
        <!-- <div class="menu">
            <ul>
                <li>
                     <a href="#home">Home</a>
                     <a href="#clubs">Clubs</a>
                     <a href="#World-Cup">World Cup 2023</a>
                     <a href="#upcoming">Upcoming Matches</a>
                     <a href="#contact">Contact</a>
                </li>
            </ul>
        </div> -->
        <div class="hero-button">
            <a href="login.php"><i class="fas fa-sign-in-alt"></i> Login</a>
        </div>
    </section>

<!-- hero section design -->
<section class="hero-section">
    <div class="hero">
<div class="hero-details">
    <h1>CUSTECH Sport Team Management</h1>
    <div class="hero-button">
        <a href="#">Watch live<i class="fas fa-long-arrow-alt-right"></i></a>
    </div>
</div>
<div class="hero-image">
    <img src="images/banner.png" alt="">
</div>
    </div>
    
</section>

<!-- player section design  -->
<div class="player-title">
    <h1>Top Players</h1>
    <hr>
    <p>Lorem ipsum dolor sit amet consectetur.</p>  
</div>
<section class="player-container">
    <div class="player">
        <img src="images/players/player-1.png" alt="">
        <h1>Neymar Jr</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-2.png" alt="">
        <h1>Lionel Messi</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-3.png" alt="">
        <h1>Cristiano Ronaldo</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-4.png" alt="">
        <h1>Paulo Dybala</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-5.png" alt="">
        <h1>Mesut Ozil</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-6.png" alt="">
        <h1>Mauro Icardi</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-7.png" alt="">
        <h1>Di Maria</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-8.png" alt="">
        <h1>Kylian Mbappé</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-9.png" alt="">
        <h1>Mohamed Salah</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-10.png" alt="">
        <h1>Harry Kane</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-11.png" alt="">
        <h1>Kevin De Bruyne</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="player">
        <img src="images/players/player-12.png" alt="">
        <h1>Philippe Coutinho</h1>
        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Lacus, fermentum amet faucibus sed id nisi lectus at.</p>
        <div class="hero-button">
            <a href="#">See More<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
</section>

<section class="about-us">
    <div class="about">
        <h1>Highlight of the Vice Chancellor CUP 2023</h1>
        <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint. Velit officia consequat duis enim velit
        mollit. Exercitation veniam consequat sunt nostrud amet.</p>
        <div class="hero-button">
            <a href="#">Checkout<i class="fas fa-long-arrow-alt-right"></i></a>
        </div>
    </div>
    <div class="about-image">
        <img src="images/match.png" alt="">
    </div>
</section>

<section class="footer">
    <div class="footer-logo">
        <img src="images/Logo.png" alt="">
    </div>
    <div class="social">
        <img src="images/Social Icons/Instagram.png" alt="">
        <img src="images/Social Icons/Dribbble.png" alt="">
        <img src="images/Social Icons/Twitter.png" alt="">
        <img src="images/Social Icons/Youtube.png" alt="">
    </div>
    <div class="copyright">
        <p>Copyright © 2025 Design by Mubynn</p>
    </div>
</section>
</body>
</html>